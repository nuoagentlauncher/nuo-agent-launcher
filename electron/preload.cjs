...
contextBridge.exposeInMainWorld('nal', {
  ...
  // 启动失败诊断 + 一键修复 + AI 崩溃分析
  diagnose: {
    run: (versionId) => invoke('diagnose:run', versionId),
    fix: (versionId, fixKey, extra) => invoke('diagnose:fix', versionId, fixKey, extra),
    analyze: (versionId) => invoke('diagnose:analyze', versionId),
  },
})
