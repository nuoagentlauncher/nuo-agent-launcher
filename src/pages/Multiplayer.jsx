    ...
    <div className="scroll-y" style={{ height: '100%', padding: '32px' }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                {currentRoom.isHost && upnp && (
                  <div className="text-xs mt-2" style={{ color: upnp.success ? 'var(--success)' : 'var(--warning)' }}>
                    {upnp.success
                      ? <>🌐 UPnP 穿透已开启：公网地址 <code style={{ fontFamily: 'Consolas' }}>{upnp.externalIp}:{upnp.externalPort}</code>，异地好友可直连此地址</>
                      : <>🌐 UPnP 穿透未生效：{upnp.note}（同一 WiFi/局域网联机不受影响）</>}
                  </div>
                )}
              </div>
                  ...
                  {recent.map((r, i) => (
                    <div key={i} style={{ padding: '10px 12px', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        {r.hasPassword && <span className="badge badge-warning ml-2">🔒 密码</span>}
                      </div>
        ...
        <div className="card mt-4" style={{ background: 'rgba(79,158,255,0.05)' }}>
          <h3 className="text-sm" style={{ fontWeight: 600, marginBottom: 8 }}>📌 联机使用说明</h3>
