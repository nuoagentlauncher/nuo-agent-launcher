...
const SECTIONS = [
  ...
  { id: 'java', name: 'Java & 启动', icon: '☕' },
  { id: 'account', name: '账号', icon: '👤' },
  { id: 'ai', name: 'AI 助手', icon: '✨' },
  { id: 'about', name: '数据 & 关于', icon: '📁' },
]
  ...
  return (
    <div style={{ height: '100%', display: 'flex' }}>
      ...
      <aside style={{ width: 220, borderRight: '1px solid var(--border-color)', padding: '16px 8px', background: 'var(--bg-secondary)' }}>
        ...
        {saved && <div className="text-xs" style={{ color: 'var(--success)', padding: '12px', textAlign: 'center' }}>✓ 已保存</div>}
      </aside>
    ...
    <Section title="AI 助手设置">
      <Field label="连接状态" hint="内置免费通道，无需任何 API 密钥，开箱即用">
        <button className="btn btn-ghost" onClick={speedTest} disabled={testing}>
          {testing ? <><span className="spinner" />测速中</> : '⚡ AI 测速'}
        </button>
        {testResult && (
          <div className="text-sm mt-4" style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 'var(--radius)', display: 'grid', gap: 6 }}>
            {testResult.success ? (
              <div style={{ color: 'var(--success)' }}>
                ✓ 最快通道：{testResult.fastest.source} · {testResult.fastest.model}（{testResult.fastest.latencyMs}ms），已自动优先生效
              </div>
            ) : (
              <div style={{ color: 'var(--error)' }}>✗ 所有通道暂时不可用，请稍后重试</div>
            )}
