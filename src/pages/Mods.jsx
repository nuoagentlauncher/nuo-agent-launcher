              ...
              <div key={m.projectId} className="card" style={{ padding: 14, display: 'flex', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span className="text-tertiary" style={{ fontSize: 11, display: 'inline-flex', alignItems: 'center', gap: 3 }}><Icon name="download" size={12} />{fmtNum(m.downloads)}</span>
                  </div>
          ...
          <div className="card" style={{ width: 560, maxHeight: '80vh', padding: 20, overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div className="text-tertiary" style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}><Icon name="download" size={13} />{fmtNum(detail.downloads)} · <Icon name="heart" size={12} />{fmtNum(detail.follows)} · {fmtDate(detail.dateModified)} 更新</div>
              </div>
              <button className="btn btn-ghost btn-sm" onClick={() => setDetail(null)}><Icon name="close" size={15} /></button>
            </div>
                ...
                <div key={v.id} className="card" style={{ padding: 10, display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg-tertiary)' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className="text-tertiary" style={{ fontSize: 11 }}>
                      {v.loaders.join('/')} · <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, verticalAlign: 'middle' }}><Icon name="download" size={11} />{fmtNum(v.downloads)}</span> · {fmtDate(v.datePublished)}
